...
export default function Payments() {
  const list = new Ref();
  const paymentMethod = new Ref();
  const $paymentDialog = <DialogBox oncancel={(hide) => hide()}>
    <table ref={paymentMethod} className='payment-method'></table>
  </DialogBox>;
  ...
  function Payment({
    ...
    return <li
      ...
      onclick={() => renderPaymentMethod(id, paymentId)}
    >
      <div className='group'>
        <strong onclick={updateStatus} data-id={id} className='status'>{status}</strong>
  ...
  async function renderPaymentMethod(id, pmId) {
    try {
      ...
      const data = await fetch(`/api/admin/payment-method/${pmId}`)
        .then((res) => res.json());
          ...
          data.bank_account_number
            ? <tr>
              ...
              <td>
                ...
                <br />
                <span>{data.bank_account_type} ({data.bank_account_holder})</span>
                <br />
