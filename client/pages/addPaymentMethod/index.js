...
export default async function AddPaymentMethod({ mode }) {
  ...
  if (mode === 'paypal') {
    ...
    form.append(<div>
      ...
      <div style={{ marginTop: '20px' }} ><sup>*</sup> are required.</div>
      <div className='buttons-container'>
        <button type='submit'>{buttonText}</button>
        <button type='reset' className='danger'>Reset</button>
      </div>
    </div>);
  } else {
    form.append(<div>
  ...
  return <section id='add-payment-method'>
    <h1>{title}</h1>
    <AjaxForm
      ref={form}
      ...
      onerror={(error) => { errorText.value = error; }}
    ></AjaxForm>
  </section>;
}
