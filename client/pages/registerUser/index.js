...
export default async function registerUser({ mode }) {
  ...
  return <section id="register-user">
    ...
    >
      ...
      <div>
        <fieldset>
          <Input autofill={false} value={user.email} onchange={(e) => { email = e.target.value; }} type="email" name="email" label='Email' placeholder='e.g. john@gmail.com' />
      ...
      <button type="submit">{buttonText}</button>
      {
        mode === 'edit'
          ? <></>
          : <a className='link' href="/login">Login to existing account.</a>
      }
    </AjaxForm>
    <p>By clicking <strong>Register</strong> button above you agree to our <a href='/policy'>Privacy Policy and Terms and conditions</a>.</p>
  </section>;
