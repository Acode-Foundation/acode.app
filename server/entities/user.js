...
const buildActiveWhere = require('../lib/buildActiveWhere');
...
class User extends Entity {
  ...
  async delete() {
    throw new Error('Hard-deleting users is disabled. Use the user anonymization service.');
  }

  activeWhere(where = [], operator = 'AND') {
    return buildActiveWhere(this.DELETED_AT, where, operator);
  }

  getActive(columns, where = [], options) {
    const operator = typeof options === 'string' ? options : options?.operator;
    return this.get(columns, this.activeWhere(where, operator), options);
  }

  countActive(where = [], options = 'AND') {
    return this.count(this.activeWhere(where, options), options);
  }

  updateActive(columns, where = [], operator = 'AND') {
    return this.update(columns, this.activeWhere(where, operator), operator);
  }
    ...
    switch (filter) {
      case 'with_plugins':
        sql = `SELECT DISTINCT u.name, u.email FROM user u
          ...
          WHERE u.role != 'admin' AND u.deleted_at IS NULL AND p.status != 3`;
        break;
      case 'with_paid_plugins':
        sql = `SELECT DISTINCT u.name, u.email FROM user u
          INNER JOIN plugin p ON u.id = p.user_id
          WHERE u.role != 'admin' AND u.deleted_at IS NULL AND p.price > 0 AND p.status != 3`;
        break;
      case 'with_payment':
        sql = `SELECT DISTINCT u.name, u.email FROM user u
          INNER JOIN payment pay ON u.id = pay.user_id
          WHERE u.role != 'admin' AND u.deleted_at IS NULL AND pay.status = 1`;
        break;
      default:
        sql = `SELECT name, email FROM user WHERE role != 'admin' AND deleted_at IS NULL`;
    }
  ...
  async countUsersByFilter(filter) {
    ...
    switch (filter) {
      ...
      case 'with_payment':
        ...
        break;
      default:
        sql = `SELECT COUNT(*) as count FROM user WHERE role != 'admin' AND deleted_at IS NULL`;
    }
